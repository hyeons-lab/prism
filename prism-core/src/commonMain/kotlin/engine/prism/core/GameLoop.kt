...
class GameLoop {
    ...
    /**
     * Advances the loop by a single frame. Useful for platforms that manage
     * their own run-loop (e.g., browser requestAnimationFrame).
     */
    fun tick() {
        if (!isRunning) return
    ...
    /**
     * Prepares the loop for external tick-based usage without entering
     * the internal while-loop. Call [tick] each frame after this.
     */
    fun startExternal() {
        if (isRunning) return
        isRunning = true
