 ...
 *
 * Provides cross-platform GPU rendering for desktop (Vulkan/Metal/DX12),
 * web (WebGPU), and mobile (Metal/Vulkan) platforms.
 *
...
class WgpuRenderer(
    ...
    override fun bindPipeline(pipeline: RenderPipeline) {
        ...
        val wgpuPipeline =
            pipeline.handle as? GPURenderPipeline
                ?: error(
                    "Pipeline handle is not a GPURenderPipeline — call createPipeline() first"
                )
                ...
                BindGroupDescriptor(
                    ...
                    entries =
                        listOf(
                            BindGroupEntry(
                                binding = 0u,
                                resource = BufferBinding(buffer = ub),
                            ),
                            BindGroupEntry(
                                binding = 1u,
                                resource = BufferBinding(buffer = mb),
                            ),
                        ),
                )
            )
                ...
                RenderPipelineDescriptor(
                        ...
                        VertexState(
                            ...
                            buffers =
                                listOf(
                                    VertexBufferLayout(
                                        arrayStride =
                                            descriptor.vertexLayout.stride.toULong(),
                    ...
                    fragment =
                        FragmentState(
                            ...
                            targets =
                                listOf(
                                    ColorTargetState(format = renderingContext.textureFormat)
                                ),
                        ),
                    primitive =
                        PrimitiveState(
    ...
    override fun uploadMesh(mesh: Mesh) {
        ...
        val vertexBuffer =
            createBuffer(BufferUsage.VERTEX, vertexSizeBytes, "${mesh.label} Vertices")
        ...
        if (mesh.isIndexed) {
            ...
            val indexBuffer =
                createBuffer(BufferUsage.INDEX, indexSizeBytes, "${mesh.label} Indices")
