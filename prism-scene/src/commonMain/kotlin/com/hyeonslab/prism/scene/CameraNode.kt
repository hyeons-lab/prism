...
class CameraNode(name: String = "CameraNode", val camera: Camera = Camera()) : Node(name) {
  ...
  override fun update(deltaTime: Float) {
    ...
    val forward = transform.rotation.rotateVec3(com.hyeonslab.prism.math.Vec3.FORWARD)
    camera.target = transform.position + forward
    camera.up = transform.rotation.rotateVec3(com.hyeonslab.prism.math.Vec3.UP)
