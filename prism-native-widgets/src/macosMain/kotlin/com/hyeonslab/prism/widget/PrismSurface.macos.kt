...
/** Creates a [PrismSurface] backed by a GLFW/Metal window with a ready-to-use wgpu context. */
suspend fun createPrismSurface(width: Int, height: Int, title: String = "Prism"): PrismSurface {
  log.i { "Creating GLFW/Metal surface: ${width}x${height}" }
  val ctx = glfwContextRenderer(width = width, height = height, title = title)
  log.i { "wgpu surface ready (macOS native)" }
