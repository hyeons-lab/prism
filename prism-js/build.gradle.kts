...
abstract class NodeDirValueSource : ValueSource<String, NodeDirValueSource.Parameters> {
    ...
    override fun obtain(): String {
        ...
        return nodejsRoot.listFiles()
            ?.filter { it.isDirectory && it.name.startsWith("node-v") && it.name.endsWith("$osName-$arch") }
            ?.maxByOrNull { it.name }
            ?.absolutePath
...
/**
 * Copies the Kotlin-generated prism.d.mts into the project directory so that
 * tsc (nodenext resolution) can find it as the type declarations for ./prism.mjs.
 *
 * Declares a specific @OutputFile (not the entire project directory) to avoid
 * Gradle's implicit-dependency validation error.
 */
abstract class CopyPrismDmtsTask @javax.inject.Inject constructor(
    private val fs: org.gradle.api.file.FileSystemOperations,
) : DefaultTask() {
    @get:InputFile
    abstract val sourceDmts: RegularFileProperty

    @get:OutputFile
    abstract val destDmts: RegularFileProperty
...
abstract class SdkNpmInstallTask @javax.inject.Inject constructor(
    private val execOps: org.gradle.process.ExecOperations,
) : DefaultTask() {
    @get:Input
    abstract val nodeDir: Property<String>
...
abstract class GenerateSdkTypesTask @javax.inject.Inject constructor(
    private val execOps: org.gradle.process.ExecOperations,
) : DefaultTask() {
    @get:Input
    abstract val nodeDir: Property<String>
...
val nodeDirProvider = providers.of(NodeDirValueSource::class) {
...
val copyPrismDmts by tasks.registering(CopyPrismDmtsTask::class) {
    dependsOn("compileProductionExecutableKotlinWasmJs")
    sourceDmts.set(layout.buildDirectory.file(
        "compileSync/wasmJs/main/productionExecutable/kotlin/prism.d.mts"
    ))
    destDmts.set(layout.projectDirectory.file("prism.d.mts"))
}

// Step 2: npm install — installs typescript into prism-js/node_modules/
val sdkNpmInstall by tasks.registering(SdkNpmInstallTask::class) {
...
val generateSdkTypes by tasks.registering(GenerateSdkTypesTask::class) {
