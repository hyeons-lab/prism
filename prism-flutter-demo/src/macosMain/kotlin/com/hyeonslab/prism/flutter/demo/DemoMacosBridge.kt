...
/**
 * macOS demo bridge. Extends [PrismMetalBridge] with the Damaged Helmet glTF scene. Throws if the
 * asset is not found in the app bundle.
 *
 * Swift usage: let bridge = DemoMacosBridge() bridge.attachMetalLayer(layerPtr: rawPtr, width: w,
 * height: h) // first draw bridge.renderFrame() // each display-link tick bridge.detachSurface() //
 * on deinit
 */
class DemoMacosBridge : PrismMetalBridge<DemoScene, DemoStore>(DemoStore()) {
   ...
   * (elevation). Called from Swift's `mouseDragged(with:)`.
   */
  ...
  /**
   * Adjusts the orbit radius by [delta] units. Positive delta zooms in; negative zooms out. Called
   * from Swift's `scrollWheel(with:)`.
   */
  ...
  /**
   * Returns the full demo state for the Flutter `getState` method channel call. Overrides the base
   * [PrismMetalBridge.getState] so the macOS response matches the Kotlin
   * [FlutterMethodHandler.getState] used by Android.
   */
  ...
  override fun shutdown() {
    runBlocking { backgroundScope.coroutineContext[Job]?.children?.forEach { it.join() } }
