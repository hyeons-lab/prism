...
/**
 * Flutter MethodChannel dispatcher for the demo. Extends [AbstractFlutterMethodHandler] so the
 * Android plugin can hold it by the abstract type without importing demo classes.
 */
class FlutterMethodHandler(private val demoBridge: PrismBridge<DemoScene, DemoStore>) :
  ...
  override fun handleDomainCall(method: String, args: Map<String, Any?>): Any? =
    when (method) {
      "togglePause" -> {
        demoBridge.store.dispatch(DemoIntent.TogglePause)
        true
      }
      "setRotationSpeed" -> {
        val speed =
          (args["speed"] as? Number)?.toFloat()
      ...
      "setMetallic" -> {
        demoBridge.store.dispatch(
          DemoIntent.SetMetallic((args["metallic"] as? Number)?.toFloat() ?: 0f)
        )
        true
      }
      "setRoughness" -> {
        demoBridge.store.dispatch(
          DemoIntent.SetRoughness((args["roughness"] as? Number)?.toFloat() ?: 0.5f)
        )
        true
      }
      "setEnvIntensity" -> {
        demoBridge.store.dispatch(
          DemoIntent.SetEnvIntensity((args["intensity"] as? Number)?.toFloat() ?: 1f)
        )
        true
      }
