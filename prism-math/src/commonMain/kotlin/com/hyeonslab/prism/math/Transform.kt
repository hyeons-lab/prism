...
data class Transform(
  ...
  /**
   * Returns a new Transform with an additional rotation applied. The new rotation is `quaternion *
   * this.rotation` so that the new rotation is applied after the existing one in world space.
   */
  fun rotate(quaternion: Quaternion): Transform =
    copy(rotation = (quaternion * rotation).normalize())
