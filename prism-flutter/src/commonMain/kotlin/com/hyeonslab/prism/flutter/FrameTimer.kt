 ...
 * - If the frame should be rendered: call [nextFrame] to increment and retrieve the counter. Skip
 *   [nextFrame] when paused so [frameCount] reflects rendered-frame count only.
 */
class FrameTimer {
  ...
  /**
   * Samples the current time, updates lastMark, and returns (deltaTime, elapsed) in seconds. Always
   * called — even when paused — so that resume doesn't compute a huge first delta.
   */
  ...
  /**
   * Increments the rendered-frame counter and returns the new count. Call only when a frame is
   * actually rendered (i.e. not paused).
   */
  ...
  /**
   * Resets only lastMark. Call after a pause so the first resumed frame doesn't compute a delta
   * covering the entire paused interval.
   */
  fun resetLastMark() {
    lastMark = TimeSource.Monotonic.markNow()
  }
