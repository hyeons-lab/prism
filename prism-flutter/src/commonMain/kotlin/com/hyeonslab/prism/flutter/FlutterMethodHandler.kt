 ...
 *
 * Handles the built-in lifecycle methods (isInitialized, shutdown, getState) and delegates
 * domain-specific calls to [handleDomainCall]. Subclasses hold a typed bridge reference to access
 * the store and dispatch events.
 */
abstract class AbstractFlutterMethodHandler(protected val bridge: PrismBridge<*, *>) {
  ...
  protected open fun getState(): Map<String, Any?> = mapOf("initialized" to bridge.isInitialized)

  /**
   * Override to handle domain-specific method calls. Access the typed store via the subclass's own
   * bridge reference. Throw [MethodNotImplementedException] for unsupported methods.
   */
