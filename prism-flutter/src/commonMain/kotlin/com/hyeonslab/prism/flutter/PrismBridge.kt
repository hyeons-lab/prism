 ...
 *
 * [T] — the scene/renderable type (e.g. DemoScene). [S] — the MVI store type, must implement
 * [Store] (e.g. DemoStore : Store<DemoUiState, DemoIntent>). Subclasses access [store] with full
 * type information for state observation and typed event dispatch.
 *
...
open class PrismBridge<T : Any, S : Store<*, *>>(val store: S) {
  ...
  internal fun attachScene(scene: T) {
    this.scene = scene
  }
