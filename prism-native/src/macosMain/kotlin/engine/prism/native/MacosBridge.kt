 ...
 *
 * [layerPtr] is the raw `CAMetalLayer *` obtained from the MTKView's `.layer`
 * property via `Unmanaged.passUnretained(layer).toOpaque()` in Swift.
 * [width] and [height] are the initial drawable dimensions in pixels.
 *
 * The engine game-loop is started in external-tick mode; call
 * [prismRenderFrame] each frame to advance time and submit a render pass.
 */
 ...
 *
 * Ticks the game-loop (updates time / fixed-update callbacks), then
 * executes a clear-color render pass via wgpu and presents the frame.
...
/**
 * Detaches the wgpu Metal surface from the engine and releases GPU resources.
 * After this call, [prismRenderFrame] is a no-op for this engine handle.
 */
