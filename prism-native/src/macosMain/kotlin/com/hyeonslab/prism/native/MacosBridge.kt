...
private data class MacosSurfaceState(val ctx: MacosContext, val width: Int, val height: Int)

private val macosSurfaces: AtomicRef<Map<Long, MacosSurfaceState>> = atomic(mapOf())
private val macosScenes: AtomicRef<Map<Long, SceneState>> = atomic(mapOf())
...
fun prismAttachMetalLayer(engineHandle: Long, layerPtr: COpaquePointer?, width: Int, height: Int) {
  ...
  macosSurfaces.update { it + (engineHandle to MacosSurfaceState(ctx, width, height)) }
    ...
    ?.let { pendingPath ->
      val scene =
        try {
          ...
          Logger.withTag("MacosBridge").e(e) {
            "Failed to auto-load pending glTF from $pendingPath"
          }
          return@let
        }
...
fun prismLoadGltfFromPath(engineHandle: Long, glbPath: CPointer<ByteVar>?) {
  ...
  val engine = Registry.get<Engine>(engineHandle) ?: return
  val surface = macosSurfaces.value[engineHandle]
  if (surface == null) {
...
fun prismRenderFrame(engineHandle: Long) {
  ...
  // Re-use a minimal clear pass — avoids importing the full render-pass DSL just for fallback.
...
fun prismResize(engineHandle: Long, width: Int, height: Int) {
  ...
  macosScenes.value[engineHandle]?.updateAspectRatio(width, height)
  // Update stored dimensions so prism_load_gltf_from_path called after a resize uses correct size.
  macosSurfaces.update { it + (engineHandle to surfaceState.copy(width = width, height = height)) }
}
...
fun prismDetachSurface(engineHandle: Long) {
  ...
  val oldScenes = macosScenes.getAndUpdate { it - engineHandle }
  oldScenes[engineHandle]?.shutdown()
  pendingGlbPaths.update { it - engineHandle }

  val oldSurfaces = macosSurfaces.getAndUpdate { it - engineHandle }
 ...
 * (elevation). Call from the macOS mouseDragged event handler.
 */
@CName("prism_orbit_by")
fun prismOrbitBy(engineHandle: Long, dx: Double, dy: Double) {
  macosScenes.value[engineHandle]?.orbitBy(dx.toFloat(), dy.toFloat())
}

/**
 * Adjusts the orbit radius by [delta] units (positive = zoom in, negative = zoom out). Call from
 * the macOS scrollWheel event handler.
 */
@CName("prism_zoom")
fun prismZoom(engineHandle: Long, delta: Double) {
  macosScenes.value[engineHandle]?.zoom(delta.toFloat())
}
...
@CName("prism_get_fps")
fun prismGetFps(engineHandle: Long): Double = macosScenes.value[engineHandle]?.fps ?: 0.0
...
fun prismTogglePause(engineHandle: Long) {
  val scene = macosScenes.value[engineHandle] ?: return
...
fun prismGetPauseState(engineHandle: Long): Int =
  if (macosScenes.value[engineHandle]?.isPaused == true) 1 else 0
...
fun prismIsRendererReady(engineHandle: Long): Int =
  if (macosScenes.value[engineHandle] != null) 1 else 0
