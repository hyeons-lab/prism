...
- `2026-03-01T22:50-0800` `prism-native/src/macosMain/.../MacosBridge.kt` — `encoder.finish().use { cmdBuf -> ... }` in fallback clear pass; closes the `GPUCommandBuffer` every frame instead of leaking it
- `2026-03-01T22:50-0800` `prism-native/src/iosMain/.../IosBridge.kt` — identical `encoder.finish().use` fix
- `2026-03-01T22:50-0800` `prism-native/src/nativeMain/.../NativeBridge.kt` — added `import kotlinx.cinterop.COpaquePointer` + `rawValue`; added 4 `*_ptr` wrapper functions (`prism_destroy_engine_ptr`, `prism_destroy_world_ptr`, `prism_destroy_scene_ptr`, `prism_destroy_node_ptr`) that take `COpaquePointer?` and forward to the `Long`-typed originals
- `2026-03-01T22:50-0800` `prism-flutter/flutter_plugin/lib/src/prism_sdk_ffi.dart` — updated all 4 `NativeFinalizer` lookups from `prism_destroy_*` to `prism_destroy_*_ptr` to match the new C signature
...
- `2026-02-24T00:00-08:00` `withContext(Dispatchers.Default)` for texture decode — per user request. Keeps GPU calls on `Dispatchers.Main` (render thread) while offloading PNG/JPEG decode to a worker thread. A `yield()` between textures lets one frame render before the next decode starts, so textures appear progressively without stalling the MTKView draw callback.
- `2026-02-24T10:30-08:00` Queue pending GLB path in native bridge — avoids the inherent race where Dart's `initState()` runs before the first `MTKView.draw(in:)`. The path is queued synchronously and consumed in `prismAttachMetalLayer`, which runs on the first draw callback. No retry loop needed on the Dart side; the scene is ready after a single `loadGltfFromPath` call.
- `2026-02-24T10:30-08:00` `Ticker` instead of `Timer.periodic` for FPS display — vsync-aligned (~60 fps) avoids redundant rebuilds (setState only when fps changes by ≥1 or ready state changes). Replaces two polling concerns (init check + FPS) with a single frame callback.
- `2026-02-24T10:30-08:00` Synchronous `fps` getter in Dart FFI layer — avoids `async/await` overhead in the per-frame callback. `prism_get_fps` is a trivial map lookup on the native side.
...
- `prism-flutter/example/macos/` was incidentally scaffolded by `flutter create` in commit 9037ebb. The correct demo scaffold lives in `prism-flutter-demo/example/`, not in the library module's example directory. Resolution: `git rm` the macos/ dir and other flutter create artifacts from `prism-flutter/example/`, delete the now-unneeded `PrismFlutterDemo` SPM local package, and strip the old bridge wiring from AppDelegate.swift + project.pbxproj.
- `prism-flutter-demo/example/lib/main.dart` used the old `PrismRenderView(engine: _engine)` API. Updated to `PrismRenderView(engineHandle: _engine.handle)` to match the updated widget signature.
- `prism-flutter/flutter_plugin/macos/.../PrismMacOSPlatformView.swift` — reduced `scrollingDeltaY` zoom sensitivity from `* 0.1` to `* 0.01`; trackpad events deliver 10–50px so 0.1 caused jumps.
- `prism-native/src/nativeMain/kotlin/com/hyeonslab/prism/native/SceneState.kt` — replaced `Dispatchers.Default` scope + `AtomicRef<List<DecodedTexture>>` upload queue + `uploadNextPendingTexture()` with `textureScope` on `Dispatchers.Main` + `withContext(Default)` for CPU decode + inline GPU upload + `yield()` between textures (GltfDemoScene pattern).
- `prism-native/src/macosMain/kotlin/com/hyeonslab/prism/native/MacosBridge.kt` — removed stale `backgroundScope` creation and `uploadNextPendingTexture()` call from `prismRenderFrame`.
- `prism-native/src/iosMain/kotlin/com/hyeonslab/prism/native/IosBridge.kt` — removed stale `CoroutineScope`, `SupervisorJob`, `cancel` imports; no `uploadNextPendingTexture()` was added here.
...
- `2026-02-25T12:26-0800` **iOS gesture signs wrong again on re-test with simulator**: After running a clean iOS simulator build with the full scene rendering, user confirmed both axes were still inverted (`+x, -y`). The earlier "corrected" conclusion was made without a fully working scene. Final confirmed fix: `-translation.x * 0.01, +translation.y * 0.01` — matches macOS convention (`-deltaX, +deltaY`). Verified working on iPhone 17 Pro Max simulator.
...
- `2026-02-25T01:15-08:00` **`registrar.lookupKey(forAsset:)` broke macOS/iOS asset resolution**: In `73811e1`, changed both Swift plugins to use `registrar.lookupKey(forAsset:)` + `FileManager.fileExists`. This caused `resolveFlutterAssetPath` to return nil on both platforms because `lookupKey` format differed from expected, or `FileManager.fileExists` returned false. Result: spinner stayed forever. Reverted in `ad3ef4a` to original hardcoded `/flutter_assets/` concatenation. Verified working by running macOS demo. Root issue: the original code always returned the path unconditionally and the demo asset is always bundled there.
- `2026-02-25T01:31-08:00` **Web Flutter demo confirmed working**: `flutter run -d chrome` launched successfully and the DamagedHelmet scene rendered in the browser.
- `2026-02-25T01:15-08:00` **Critical review found 9 bugs/regressions** introduced during this session: (1) `prism_engine_channel.dart` missing `int get handle` → Android compile failure; (2) Android `PrismRenderView` never shown due to `if (kIsWeb || handle != 0)` guard with handle always 0; (3) Android spinner never hides (`isRendererReady = false`); (4) Web `_canvasId` always null because `attachCanvas` call was removed when switching from `engine:` to `engineHandle:` parameter; (5) iOS/macOS `resolveFlutterAssetPath` hardcoded path fragile in debug builds; (6) API regression — exposing raw `int engineHandle` instead of `PrismEngine` on `PrismRenderView`; (7) `resolveFlutterAssetPath` on Android would throw `MissingPluginException`; (8) Web fps getter used static `lastCanvasId` global instead of per-instance `_canvasId`; (9) Compose Desktop controls (sliders, pause) were removed in simplification commit. All 9 fixed in `73811e1`.
- `2026-02-26T02:47-08:00` **Flutter CI failing: `uri_does_not_exist` for generated bindings**: `generated/*.dart` was gitignored via `*.dart` in the directory `.gitignore`. CI clones a clean checkout with no generated files, so `flutter analyze` failed on `prism_engine_ffi.dart` and `prism_sdk_web.dart` with 31 errors. Fix: removed `*.dart` from `generated/.gitignore` and committed both files (`prism_native_bindings.dart` 638 lines, `prism_js_bindings.dart` 113 lines). They're small and only change when C API or WASM exports change. Flutter CI now passes.
- `2026-02-25T01:15-08:00` **CI had no Flutter compile check**: Missing interface methods (like `handle` in channel) only caught at `flutter run` time. Added `flutter` CI job: `flutter pub get` + `flutter analyze --no-pub` on both plugin and demo directories. Runs on ubuntu-latest with stable Flutter SDK.
...
- a8899f6 — fix: revert Compose Desktop to ComposePanel+withFrameNanos, fix detekt violations
- 800811d — fix: correct Compose Desktop orbit axis signs to match native GLFW demo
- 8f4fb8c — feat: PrismEngineInterface contract + web engine stubs + web demo fixes
- 289089c — fix: web Flutter demo — WASM build deps, auto-rotation, FPS display
- 73811e1 — fix: address all critical review issues in Flutter plugin
- ad3ef4a — fix: revert iOS/macOS asset path to hardcoded flutter_assets/ prefix
